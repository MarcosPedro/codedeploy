...
});g(window).on("load.bs.select.data-api",function(){g(".selectpicker_DV").each(function(){var o=g(this);k.call(o,o.data())})})})(a)}));
...
function DVelop_WorkWithPlusUtilities() {
	...
	this.PorcessDetailWCs = function () {
		var thisC = this;
		$(".WCD_ActionColumn").each(function (i) {
			if (!$(this).hasClass("WCD_processed")) {
				...
				if (wcOriginalCell.children().length > 0) {
					...
					wcOriginalCell.attr('data-wcd-id', wcId);
				} else {
					wcId = wcOriginalCell.attr('data-wcd-id');
				}
				if (wcId != null && wcId != '') {
					$(this).on("click", function (e) {
						if ((thisC.isAutomaticOpen != null && thisC.isAutomaticOpen) || e.target.tagName != 'P' && e.target.tagName != 'TD') {
							if (!$(this).hasClass("WCD_Expanded")) {
								$(this).addClass("WCD_Expanded");
								if (wcId.startsWith('gxHTMLWrp')) {
								...
								$(newDiv).addClass("SectionWCDContainer");
								...
								if ($("#" + wcId).length > 0) {
									...
									if (wcParent.hasClass('SectionWCDContainer')) {
										...
										wcParent.slideUp(function () {
											$(this).closest('.WCD_tr').prev().find('.WCD_ActionColumn').removeClass("WCD_Expanded");
											$(this).closest('.WCD_tr').remove();
										});
									}

									var dateContainer = wcElem.detach();
									if (thisC.isAutomaticOpen == null || !thisC.isAutomaticOpen) {
										dateContainer.find('.gxwebcomponent-body').addClass('Invisible');
									}
									dateContainer.appendTo(newDiv);
								} else {
									var divWC = document.createElement('div');
									divWC.id = wcId;
									newDiv.appendChild(divWC);
								}
								if (showLoading) {
									...
									$(newDiv1).slideDown(function () {
										$(newDiv).css('min-height', $(this).height());
									});
								}
								window.setTimeout(function () {
									if (showLoading) {
										$(newDiv).parent().find('.WCD_Loading').remove();
									}
			...
			if ($(this).hasClass("WCD_Opened")) {
				thisC.isAutomaticOpen = true;
				$(this).removeClass("WCD_Opened");
				$(this).click();
				thisC.isAutomaticOpen = null;
			}
		});
	}
			...
			if (!$(h).hasClass("selectpicker_DV")) {
				...
				c.on("changed.bs.select", function (j, i, l, k) {
					if ($(this).find('>select').hasClass('selectpicker_DV')) {
						...
						window.setTimeout(function () {
							$(j.target).val(0);
							$(j.target).selectpicker_DV("refresh");
	...
	this.ApplyBootstrapSelect_setSectionGridMinHeight = function (ddo, setMinHeight) {
		...
		for (var i = 0; i < 18; i++) {
			if (overflowEl == null || $(overflowEl).css('overflow') == 'auto') {
				break;
			}
			else {
		...
		if ($("#" + tableTotalizerId).length > 0 && $(this).data('wwpTotRowAdded') == null) {
			...
			if ($(this).css('display') != 'none' && $(this).find('>tbody>tr').length > 0) {
				...
				$(this).find('>tbody>tr:not(.DVGroupByRow):eq(0)>td').each(function () {
					...
					newCol.innerHTML = totalizerTable.find("td:eq(" + $(this).index() + ")")[0].innerHTML;
					newRow.appendChild(newCol);
